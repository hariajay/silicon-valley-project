...
var collect,season=1,rand1,rand2,rand3,sharpgroup,btn
var dis=0,coinum=0,mins=0,gems=0,e1=10650,e2=10900,t1=1,t2=1,s=1,l=1,p=1,j=1,n=1,f=1,g=1,k=1,o=1,b=1,a=1,c=1,m=1,d=1,q=1,z=1,x=1,y=1,w=1,r=22200,u=1
var si=1,jk=1,v=1,cv=1,kg=1,py=-200,sh=1,it=1,e=1,bs=[],h=1
...
function setup(){
    ...
    world.add(wld,last)
    up1 = Bodies.rectangle(20670,330,70,140,{isStatic:true})
    world.add(wld,up1)
    up2 = Bodies.rectangle(20870,330,70,140,{isStatic:true})
    world.add(wld,up2)
    up3 = Bodies.rectangle(21070,330,70,140,{isStatic:true})
    world.add(wld,up3)
    up4 = Bodies.rectangle(21270,330,70,140,{isStatic:true})
    world.add(wld,up4)
    knife(300,560)
    knife(1115,1600)
    knife(2235,2900)
    ...
    g16 = new Ground(20000,438,460)
    g17 = new Ground(10950,415,2250)
    ...
    q1.shapeColor = "brown"
    q2.shapeColor = "brown"
    s4 = new Sharp(20460,165,sh2)
    s5 = new Sharp(20490,165,sh2)
    ...
    re = createSprite(300,-295,100,100)
    blue = createSprite(ball.x,ball.y,40,40)
...
function draw(){
  background(0)
   ...
   rect(up4.position.x,up4.position.y,70,140)
   if(jk==1){
   ...
   if(m==2){
    Matter.Body.applyForce(iron1,iron1.position,{x:0.005,y:0})
    Matter.Body.applyForce(iron2,iron2.position,{x:0.005,y:0})
   }
   if(z2.isTouching(z3)){
     m=3
  }
  ...
  wd8.display()
   b+=0.1
   if(o==1){
     image(frame,145,120,350,95)
     textSize(20)
     fill("brown")
     stroke(0)
     strokeWeight(0.5)
     text("Complete within countdown",23,145)
     text("Season1: Blue mounts",47,110)
   }
  ...
  if(keyDown("space")){
   ...
   if(keyDown("left")){
    Matter.Body.applyForce(player.body,player.body.position,{x:-2.2,y:0})
  }
   if(keyDown("right")){
    Matter.Body.applyForce(player.body,player.body.position,{x:2.2,y:0})
  }
  ...
  if(ball.x>14730){
    season=3
    //image(time,camera.position.x,265,220,195)
    it+=0.1
  }
  if(season==3 && e==1){
    image(frame,camera.position.x,113,330,90)
    textSize(19)
    fill("brown")
    stroke(0)
    strokeWeight(0.5)
    text("Complete within countdown",camera.position.x-117,135)
    text("Season2: Dark shadows",camera.position.x-100,105)
  }
  if(it>7){
    e=2
    it=1
  ...
  if(season==2 && v==1){
    ...
    strokeWeight(0.5)
    text("Complete within countdown",camera.position.x-117,135)
    text("Season2: Countryside",camera.position.x-90,105)
  }
  if(cv>7){
    v=2
    cv=1
